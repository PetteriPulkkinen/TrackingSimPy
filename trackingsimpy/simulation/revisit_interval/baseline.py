...
class Baseline(BaseRISimulation):
    ...
    def __init__(self, tracker, k_min, k_max, n_max, traj_idx, P0, beamwidth, pfa, sn0, save):
        if P0 is None:
            P0 = np.eye((self.ORDER + 1) * self.DIM) * 1000

        target = TrajectoryTarget(load_trajectory(get_file_list()[traj_idx], self.ORDER, dim=self.DIM), self.ORDER, self.DIM)
